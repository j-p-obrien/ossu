use hw2::{hamming::CodeList, hamming2::CodeBucket, union_find::EdgeList};
fn main() {
    ...
    let num_clusters = graph.cluster_optimized(3);
    println!(
        "The number of clusters with spacing of 3 is: {}",
        num_clusters
    )*/

    let graph = CodeBucket::from_file("clustering_big.txt");
