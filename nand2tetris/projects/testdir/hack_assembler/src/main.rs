...
enum Token<'a> {
    ...
    // Hacky I suppose but seems like the easiest way to delimit lines.
    Newline,
}
...
impl<'a> Lexer<'a> {
    ...
    fn skip_whitespace(&mut self) -> Option<(usize, u8)> {
        self.file_iter
            .find(|(_, b)| !b.is_ascii_whitespace() || *b == b'\n')
    }
    ...
    fn skip_comments_and_whitespace(&mut self) -> Option<(usize, u8)> {
        let (pos, byte) = self.skip_whitespace()?;
        todo!()
        // loop {
